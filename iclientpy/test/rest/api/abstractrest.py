...
from iclientpy.rest.apifactory import APIFactory, OnlineAPIFactory, iPortalAPIFactory, iManagerAPIFactory
...
class AbstractREST(object):
    ...
    @httpretty.activate
    def init_imanager_apifactory(self):
        if not hasattr(self, 'factory'):
            loginuri = self.loginuri if hasattr(self, 'loginuri') else self.baseuri + '/security/tokens.json'
            httpretty.register_uri(httpretty.POST, loginuri, status=201,
                                   set_cookie='JSESSIONID=958322873908FF9CA99B5CB443ADDD5C')
            self.factory = iManagerAPIFactory(self.baseuri, self.username, self.password)
