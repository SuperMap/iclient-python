...
from iclientpy.rest.api.onlinedatasservice import OnlineDatasService
from iclientpy.rest.api.model import *
from unittest import mock


class OnlineDatasServiceTestCase(AbstractRESTTestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_datas(self):
        ...
        self.check_api(OnlineDatasService.get_datas, self.baseuri + "/web/datas.json", HttpMethod.GET,
                       httpretty.Response(body=get_body, status=201))
        entity = PostMyDatasItem()
        entity.type = DataItemType.JSON
        entity.fileName = 'test.json'
        self.check_api(OnlineDatasService.post_datas, self.baseuri + "/web/mycontent/datas.json", HttpMethod.POST,
                       httpretty.Response(
    ...
    def test_data(self):
        ...
        self.check_api(OnlineDatasService.get_data, self.baseuri + "/web/datas/18220636.json", HttpMethod.GET,
        ...
        self.check_api(OnlineDatasService.put_data, self.baseuri + "/web/mycontent/datas/18220636.json", HttpMethod.PUT,
                       httpretty.Response(body='{"succeed":true,"newResourceID":"1"}', status=201), data_id=data_id,
                       entity=entity)
        self.check_api(OnlineDatasService.delete_data, self.baseuri + "/web/mycontent/datas/18220636.json",
                       HttpMethod.DELETE,
                       httpretty.Response(body='{"succeed":true}', status=201), data_id=data_id)
        with open('./World.zip', 'rb') as fileb:
            self.check_api(OnlineDatasService.upload_data, self.baseuri + "/web/mycontent/datas/18220636/upload.json",
                           HttpMethod.POST,
                           httpretty.Response(
        ...
        self.check_api(OnlineDatasService.get_upload_process,
                       self.baseuri + "/web/mycontent/datas/18220636/progress.json",
                       HttpMethod.GET,
                       httpretty.Response(body='{"id":"null","read":94,"total":100}', status=201), data_id=data_id)

    def test_data_sharesetting(self):
        entity = OnlineDataShareSetting()
        entity.ids = ['12']
        self.check_api(OnlineDatasService.put_sharesetting,
                       self.baseuri + "/web/mycontent/datas/sharesetting.json", HttpMethod.PUT,
                       httpretty.Response(body='{"succeed": true}', status=201), entity=entity)
        ...
        self.check_api(OnlineDatasService.put_data_sharesetting,
                       self.baseuri + "/web/mycontent/datas/629565098/sharesetting.json", HttpMethod.PUT,
                       httpretty.Response(body='{"succeed": true}', status=201), data_id='629565098', entity=[entity])
